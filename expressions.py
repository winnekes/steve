            ...
            for col in row:
                for i in range(PIXEL_SCALE):
                    for j in range(PIXEL_SCALE):
                        offset = self.get_offset(x * PIXEL_SCALE + i, y * PIXEL_SCALE + j, len(data) * PIXEL_SCALE,
                                                 len(data) * PIXEL_SCALE)
                        if i == 0 or i == PIXEL_SCALE - 1 or j == 0 or j == PIXEL_SCALE - 1:
                            self.image.set_at((x * PIXEL_SCALE + i, y * PIXEL_SCALE + j),
                                              pygame.Color(self.retro_color([130, 220, 170], offset=7)))
                            continue
                        self.image.set_at((x * PIXEL_SCALE + i, y * PIXEL_SCALE + j), pygame.Color(
                            self.retro_color([25, 26, 22], offset=offset)) if col == "O" else pygame.Color(
                            self.retro_color([130, 220, 170], offset=offset)))
                x += 1
            y += 1
            x = 0
